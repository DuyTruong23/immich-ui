<script lang="ts">
  ...
  const isMobile = $derived(mediaQueryManager.pointerCoarse);
  ...
  <div in:fade={{ duration: 500 }}>
    <div class="flex flex-col gap-6 sm:ms-8">
      ...
      <Field label={$t('video_hover_setting')} description={$t('video_hover_setting_description')}>
        ...
        {#if isMobile}
          <Text size="small" class="mt-2 text-(--pg-text-muted)">
            Thiết bị cảm ứng không phát video khi hover — tùy chọn này chỉ có hiệu lực trên máy tính.
          </Text>
        {/if}
      </Field>
