<script lang="ts">
  ...
  const classes = {
    column1: 'w-7/12 md:w-4/12',
    column2: 'hidden md:block md:w-3/12',
    column3: 'hidden lg:block lg:w-1/12',
    column4: 'hidden lg:block lg:w-1/12',
    column5: 'hidden md:block md:w-2/12',
    column6: 'w-5/12 md:w-1/12 flex justify-end',
  };
</script>
...
<AdminPageLayout breadcrumbs={[{ title: data.meta.title }]} actions={[ScanAll, Create]}>
  <Container size="large" center class="my-4 p-2 pb-16">
    <div class="flex w-full flex-col items-center gap-2" in:fade={{ duration: 500 }}>
      {#if libraries.length > 0}
        <Table striped size="small" spacing="small" class="w-full">
          <TableHeader>
