...
>
  <Container size="large" center class="p-2 pb-16">
    <div class="grid w-full grid-cols-1 gap-4 lg:grid-cols-2">
      <Heading tag="h1" size="large" class="col-span-full my-4">{library.name}</Heading>
      <div class="col-span-full flex flex-col gap-4 sm:flex-row">
          ...
          <table class="w-full">
            <tbody>
              {#each library.importPaths as folder (folder)}
                ...
                <tr class="block py-3 md:table-row md:h-12">
                  <td class="block break-all md:table-cell md:align-middle">
                    <Code>{folder}</Code>
                  </td>
                  <td class="mt-2 flex justify-end gap-2 md:mt-0 md:table-cell md:align-middle md:whitespace-nowrap">
