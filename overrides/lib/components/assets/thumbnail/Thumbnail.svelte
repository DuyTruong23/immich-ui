<script lang="ts">
  ...
  const playbackUrl = $derived.by(() => {
    if (!authManager.mediaAuthReady || !asset.isVideo) {
      return;
    }
  ...
  const livePhotoPlaybackUrl = $derived.by(() => {
    if (!authManager.mediaAuthReady || !asset.livePhotoVideoId || !shouldLoadLivePhotoPreview()) {
      return;
    }
  ...
  const gifUrl = $derived.by(() => {
    if (!authManager.mediaAuthReady || !shouldLoadAnimatedPreview()) {
      return;
    }
