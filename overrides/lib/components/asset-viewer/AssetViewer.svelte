<script lang="ts">
  ...
  const isMobileViewer = $derived(mediaQueryManager.pointerCoarse || mediaQueryManager.maxMd);
  const showDesktopArrows = $derived(
    !isMobileViewer &&
...
>
  <SwipeBackEdge
    onBack={closeViewer}
    enabled={$slideshowState === SlideshowState.None &&
      !assetViewerManager.isShowEditor &&
      !assetViewerManager.isFaceEditMode}
  />
  <!-- Top navigation bar -->
  ...
  {#if showDesktopArrows && previousAsset}
  ...
  {#if showDesktopArrows && nextAsset}
