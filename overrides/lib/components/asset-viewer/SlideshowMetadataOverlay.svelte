<script lang="ts">
  ...
  import { formatDateParts, isVietnameseLocale, resolveFormatLocale } from '$lib/utils/date-format';
  ...
  const dateString = $derived(
    isVietnameseLocale($locale)
      ...
      : dateTime.toLocaleString(DateTime.DATE_MED_WITH_WEEKDAY, { locale: resolveFormatLocale($locale) }),
  );
