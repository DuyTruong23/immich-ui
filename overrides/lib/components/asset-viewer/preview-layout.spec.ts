...
describe("containFitFrame", () => {
  ...
  it("returns null when media or container size is missing", () => {
    expect(
      containFitFrame({ width: 0, height: 100 }, { width: 400, height: 800 }),
    ).toBeNull();
    expect(
      containFitFrame({ width: 100, height: 100 }, { width: 0, height: 800 }),
    ).toBeNull();
  });
});
...
describe("buildPreviewStrip", () => {
  ...
  it("returns a single item when the gallery has one asset", () => {
    expect(
      buildPreviewStrip({ current: item("1") }).map((entry) => entry.id),
    ).toEqual(["1"]);
  });
  ...
  it("forces previous/next beside current when the iterator order is stale", () => {
    ...
    expect(strip.map((entry) => entry.id)).toEqual([
      "stale-left",
      "real-prev",
      "c",
      "real-next",
...
describe("windowPreviewStrip", () => {
  ...
  it("windows the middle of a long list to five items", () => {
    expect(windowPreviewStrip(assets, "4").map((entry) => entry.id)).toEqual([
      "2",
      "3",
      "4",
      "5",
  ...
  it("does not pad the start or end to five items", () => {
    expect(windowPreviewStrip(assets, "1").map((entry) => entry.id)).toEqual([
      "1",
      "2",
      "3",
    ]);
    expect(windowPreviewStrip(assets, "7").map((entry) => entry.id)).toEqual([
      "5",
      "6",
      "7",
    ]);
