<script lang="ts">
  ...
  import { formatDate, isVietnameseLocale, resolveFormatLocale } from '$lib/utils/date-format';
  ...
  const dateLocaleString = (dateString: string) => {
    ...
    return date.toLocaleDateString(resolveFormatLocale($locale), dateFormats.album);
  };
