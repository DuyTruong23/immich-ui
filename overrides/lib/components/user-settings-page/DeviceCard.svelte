<script lang="ts">
  ...
  import { formatDateTime, resolveFormatLocale } from '$lib/utils/date-format';
  ...
  const options = $derived<ToRelativeCalendarOptions>({
    unit: 'days',
  ...
  <div class="flex grow flex-row justify-between gap-1 ps-4 sm:ps-0">
    <div class="flex flex-col justify-center gap-1 dark:text-white">
      ...
      <div class="text-sm">
        ...
        <span>{DateTime.fromISO(session.updatedAt, { locale: resolvedLocale }).toRelativeCalendar(options)}</span>
        <span class="text-xs text-gray-500 dark:text-gray-400"> - </span>
        <span class="text-xs text-gray-500 dark:text-gray-400">
          {formatDateTime(new Date(session.updatedAt), $locale)}
        </span>
      </div>
    </div>
