...
import { formatDate, formatDateTimeParts, isVietnameseLocale } from '$lib/utils/date-format';
...
const formatISODateToLocale = (iso: string, locale: string | undefined): string => {
  const date = fromISODateTimeUTC(iso);
  if (isVietnameseLocale(locale)) {
    return formatDate(date.toJSDate(), locale);
  }
  return date.toLocaleString({ month: 'short', day: 'numeric', year: 'numeric' }, { locale });
};
...
const metadataFields = [
  ...
  },
  {
    ...
    render: (asset, $t, locale) => {
      ...
      if (isVietnameseLocale(locale)) {
        const formatted = formatDateTimeParts(
          {
            year: dateTime.year,
        ...
        return dateTime.offsetNameShort ? `${formatted} ${dateTime.offsetNameShort}` : formatted;
      }
