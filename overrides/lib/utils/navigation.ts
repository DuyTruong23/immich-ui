...
/** Đóng viewer hoặc đổi ảnh: replace history để vuốt back = mũi tên, không stack/reload. */
function withViewerHistory(assetId: string | null | undefined, options?: NavOptions): NavOptions | undefined {
  const shouldReplace = !assetId || Boolean(page.params.assetId);
  if (!shouldReplace) {
    return options;
  }
...
async function navigateAssetRoute(route: AssetRoute, options?: NavOptions) {
  ...
  const navOptions = withViewerHistory(assetId, options);
  if (next !== current || navOptions?.forceNavigate) {
    await goto(next, navOptions);
  }
}
