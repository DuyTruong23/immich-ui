<script lang="ts">
  ...
  import { Button, Field, FormModal, HelperText } from '@immich/ui';
  ...
  let birthDate = $state(person.birthDate ? DateTime.fromISO(person.birthDate) : undefined);
...
<FormModal title={$t('set_date_of_birth')} size="small" icon={mdiCake} {onClose} {onSubmit}>
  <div class="my-2 flex flex-col gap-2">
    <Field label={$t('date_of_birth')}>
      <DateTimeField bind:value={birthDate} maxDate={DateTime.now()} />
