...
<Modal title={$t('about')} {onClose}>
  <ModalBody>
    <div class="flex flex-col gap-4 sm:grid sm:grid-cols-2">
      ...
      <ServerAboutItem id="immich" title="WeGallery" version={info.version} versionHref={info.versionUrl} />
          ...
          {#each versions.slice(0, 5) as item (item.id)}
            ...
            <li>
              <span
                ...
                id="version-history"
                title={formatDateTime(createdAt.toJSDate(), $locale)}
              >
                {$t('version_history_item', {
                  values: {
                    version: item.version,
                    date: formatDate(createdAt.toJSDate(), $locale),
                  },
                })}
              </span>
            </li>
