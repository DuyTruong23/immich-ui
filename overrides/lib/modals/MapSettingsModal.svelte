    ...
    {#if customDateRange}
      <div in:fly={{ y: 10, duration: 200 }} class="flex flex-col gap-4">
        <Field label={$t('date_after')}>
          <DateTimeField
            value={settings.dateAfter ? DateTime.fromISO(settings.dateAfter) : undefined}
            maxDate={settings.dateBefore ? DateTime.fromISO(settings.dateBefore) : undefined}
            onChange={(date) => (settings.dateAfter = date?.toUTC().toISO() ?? undefined)}
          />
        </Field>
        <Field label={$t('date_before')}>
          <DateTimeField
            value={settings.dateBefore ? DateTime.fromISO(settings.dateBefore) : undefined}
