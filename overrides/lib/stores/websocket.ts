...
/** Vercel không proxy WebSocket — kết nối trực tiếp tunnel Immich khi set PUBLIC_IMMICH_WS_URL */
const wsOrigin = PUBLIC_IMMICH_WS_URL.trim() || undefined;

const websocket: Socket<Events> = io(wsOrigin, {
  path: '/api/socket.io',
  transports: ['websocket'],
  withCredentials: !!wsOrigin,
  reconnection: true,
  forceNew: true,
  autoConnect: false,
});
