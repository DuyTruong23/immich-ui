...
export const fromISODateTime = (isoDateTime: string, timeZone: string): DateTime<true> =>
  DateTime.fromISO(isoDateTime, { zone: timeZone, locale: luxonLocale() }) as DateTime<true>;
...
export const fromTimelinePlainDateTime = (timelineDateTime: TimelineDateTime): DateTime<true> =>
  DateTime.fromObject(timelineDateTime, { zone: 'local', locale: luxonLocale() }) as DateTime<true>;
...
export function formatTimelineMonthTitle(_date: DateTime): string {
  ...
  if (isVietnameseLocale(stored)) {
    return formatMonthYearParts({ year: date.year, month: date.month }, stored);
  }
...
export function formatGroupTitle(_date: DateTime): string {
  ...
  const stored = get(locale);
  const resolved = luxonLocale();

  // Today
  if (today.hasSame(date, 'day')) {
    return date.toRelativeCalendar({ locale: resolved });
  }

  // Yesterday
  if (today.minus({ days: 1 }).hasSame(date, 'day')) {
    return date.toRelativeCalendar({ locale: resolved, unit: 'days' });
  }

  // Last week
  if (date >= today.minus({ days: 6 }) && date < today) {
    return date.toLocaleString({ weekday: 'long' }, { locale: resolved });
  }

  if (isVietnameseLocale(stored)) {
    return formatDateParts({ year: date.year, month: date.month, day: date.day }, stored);
  }
  ...
  return getDateLocaleString(date, { locale: resolved });
}

export const getDateLocaleString = (date: DateTime, opts?: LocaleOptions): string => {
  const stored = opts?.locale ?? get(locale);
  if (date.isValid && isVietnameseLocale(stored)) {
    return formatDateParts({ year: date.year, month: date.month, day: date.day }, stored);
  }
  return date.toLocaleString(DateTime.DATE_MED_WITH_WEEKDAY, {
    ...opts,
    locale: typeof stored === 'string' ? resolveFormatLocale(stored) : stored,
  });
};
