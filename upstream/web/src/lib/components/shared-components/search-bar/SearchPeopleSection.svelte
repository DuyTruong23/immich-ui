<script lang="ts">
  ...
  const styles = tv({
    base: 'pg-people-selection-item flex flex-col items-center rounded-3xl border-2 p-2 transition-all hover:bg-subtle dark:hover:bg-immich-dark-primary/20',
    variants: {
      selected: {
...
{#await peoplePromise}
  <div id="spinner" class="flex h-54 items-center justify-center">
...
{:then people}
  {#if people && people.length > 0}
    ...
    <div
      id="people-selection"
      class="pg-people-selection {showAllPeople ? 'pg-people-selection--expanded' : 'pg-people-selection--collapsed'}"
    >
      <div class="pg-people-selection__header">
        <Text class="shrink-0 py-1" fontWeight="medium">{$t('people')}</Text>
        <div class="pg-people-filter min-w-0 flex-1">
          <SearchBar bind:name placeholder={$t('filter_people')} showLoadingSpinner={false} />
        </div>
      </div>

      <SingleGridRow
        class="pg-people-selection__grid mt-2 grid grid-auto-fill-20 gap-1"
      ...
      {#if showAllPeople || people.length > peopleList.length}
        <div class="pg-people-selection__toggle mt-2 flex justify-center">
          <Button
            color="primary"
            variant="ghost"
            shape="round"
